...
//
// ## Deferred logging - DeferredLoggingConsumer
// For HTTP requests, everything is handled by the consumer. Once the consumer is done, it can log:
// - Request details
// - Response details
// - Response body
// For WS requests, everything is handled by the after method
//
// ## Immediate logging - ??? - TBD - TODO
// The interceptor has multiple entry points
// - Request
//   Handled in the before method
// - Response
//   Handled in the after method
// - Response body
//   Handled in by the consumer
// For WS requests, we might need to look into further improving the StandardHttpClient implementation
  ...
  private static final class HttpLogger {
    ...
    void logStart() {
      if (logger.isTraceEnabled()) {
        logger.trace("-HTTP START-"); // TODO - Decide the format
      }
    }

    void logEnd() {
      if (logger.isTraceEnabled()) {
        logger.trace("-HTTP END-"); // TODO - Decide the format
      }
    }

    void logWsStart() {
      if (logger.isTraceEnabled()) {
        logger.trace("-WS START-"); // TODO - Decide the format
      }
    }

    void logWsEnd() {
      if (logger.isTraceEnabled()) {
        logger.trace("-WS END-"); // TODO - Decide the format
      }
    }
  }
}
